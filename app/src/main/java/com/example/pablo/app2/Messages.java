...
public class Messages extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        final ListView listView = (ListView)v.findViewById(R.id.listViewMessages);
    ...
    public void setUserVisibleHint(boolean isVisibleToUser) {
        ...
        if (isVisibleToUser) {
            adapter.clear();
            String username="";
            if(((MyActivity) this.getActivity()).team.length()>0) {
                username = "[" + ((MyActivity) this.getActivity()).team + "]" + ((MyActivity) this.getActivity()).username;
            }else{
                username = ((MyActivity) this.getActivity()).username;

            }

            List<MessageDB> messages = Select.from(MessageDB.class)
                    .where(Condition.prop("tousername").eq(username)).orderBy("-id").list();
